...
func TestCreateScheduleNilRequestDoesNotPanic(t *testing.T) {
	server := NewNzovuServer(&contractStorage{}, nil, log.NewLogger())
...
func TestValidateCalendarScheduleReturnsStructuredIssue(t *testing.T) {
	server := NewNzovuServer(&contractStorage{validationErr: errors.New("rule 2 validation failed")}, nil, log.NewLogger())
...
func TestValidateCalendarScheduleAcceptsValidSchedule(t *testing.T) {
	...
	server := NewNzovuServer(storage, nil, log.NewLogger())
...
func TestRegisterSchemaRejectsUnsupportedContentType(t *testing.T) {
	...
	server := NewNzovuServer(&contractStorage{}, registry, log.NewLogger())
