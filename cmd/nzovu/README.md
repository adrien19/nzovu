...
The shipped command is `nzovu`; CLI and UI source lives in `cmd/nzovu`. Build from the repository root:
...
`--insecure` disables transport TLS for local development. For TLS use `--ca-file`; mTLS additionally requires `--cert-file` and `--key-file`. Supply an API key with `--api-key` or `NZOVU_API_KEY`.
...
Messages accept inline JSON, `--file`, or stdin (`-`). Ordered binary headers use repeatable `--header key=value` or `--header key=base64:encoded-value`. The internal header prefix `x-nzovu-` is reserved. Bulk input and ownership rules are documented in [API validation](../../API_VALIDATION.md).
